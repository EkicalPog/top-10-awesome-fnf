...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'void':
				{
					defaultCamZoom = 0.55;
					curStage = 'void';
					var bg:FlxSprite = new FlxSprite(-800,-600).loadGraphic(Paths.image('top10awesome/thefuckingvoid'));
					bg.antialiasing = true;
		...
		healthBar.createFilledBar(0xFFFF1500, 0xFF000EFF);
		// healthBar
		add(healthBar);

		// Add Kade Engine watermark
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Awesome" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - Friday Night Maker (KE) " + MainMenuState.kadeEngineVer : ""), 16);
		...
		botPlayState = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (FlxG.save.data.downscroll ? 100 : -100), 0, "SHITPLAY", 20);
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				#end
				camFollow.setPosition(boyfriend.getMidpoint().x - 100 + offsetX + bfnoteMovementXoffset, boyfriend.getMidpoint().y - 150 + offsetY + bfnoteMovementYoffset);
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						switch (Math.abs(daNote.noteData))
						{
							case 2:
								...
								dadnoteMovementYoffset = -35;
							case 3:
								dad.playAnim('singRIGHT' + altAnim, true);
								dadnoteMovementXoffset = 35;
							case 1:
	...
	*/
				...
				if (!note.wasGoodHit)
				{
					...
					switch (note.noteData)
					{
						case 2:
							...
							bfnoteMovementYoffset = -35;
						case 3:
							boyfriend.playAnim('singRIGHT', true);
							bfnoteMovementXoffset = 35;
						case 1:
	...
	{
		if (curSong == 'Awesomeness') 
		{
			switch (curStep)
			{
				case 368:
					FlxTween.tween(FlxG.camera, {zoom: 0.8}, 1.2, {ease: FlxEase.expoOut,});
				case 369:
					dad.playAnim('gun', false);
				case 384:
					changeDad('gun10awesome');
					dad.y += 24;
				case 623:
					defaultCamZoom = 1.3;
				case 639:
					changeDad('top10awesome');
					dad.y -= 24;
					defaultCamZoom = 0.55;
			}
		}
	...
	override function beatHit()
	{
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			if (SONG.notes[Math.floor(curStep / 16)].mustHitSection && dad.curCharacter != 'gf' && ((dad.curCharacter == 'top10awesome') && (!dad.animation.curAnim.name.startsWith("gun"))))
				dad.dance();
		}
